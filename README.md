...
| `build-exe.bat` | optional | - | Builds `dist\findex.exe` - one file that needs no Python. Only if you want to hand findex to someone. |
...
in. The build scripts look after their own prerequisites: on Windows a real
Python is found or installed; on the Mac an interpreter that can actually be
bundled is chosen (Apple's `/usr/bin/python3` is refused - its Tk cannot be
copied into an app that opens) and, if there is none, Homebrew installs one,
installing Homebrew itself first if needed (that step asks for your password
once). The result runs anywhere and keeps its index next to itself - beside
`findex.exe`, and beside (not inside) `findex.app` - so it is just as portable
as the folder. Each build ends by running the built app's own self-test and
says PROBLEMS FOUND rather than Done if something is missing; the full run is
in `build-win-log.txt` / `build-mac-log.txt`.
